  ...
  items.forEach((item) => {
    ...
    if (item.type === ITEM_TYPES.FOLDER) {
      ...
      itemDiv.innerHTML = `
                ...
                <span style="color:var(--text-dim); font-size:12px; flex-grow:1;">${
      ...
      itemDiv.innerHTML = `
                ...
                <input type="url" class="bookmark-url-input" value="${
                  item.href || ''
                }" placeholder="URL">
                <input type="date" class="date-input" value="${isoDate}">
            `;
...
export function renderSearchResults(results) {
  ...
  results.forEach((item) => {
    ...
    const iconName = isFolder ? 'folder' : 'bookmark';
    const iconColor = isFolder ? 'var(--folder-color)' : 'var(--item-color)';
    ...
    itemDiv.innerHTML = `
            ...
            <div style="display:flex; flex-direction:column; overflow:hidden;">
                ...
                <span style="font-size:10px; color:var(--text-dim); margin-left:8px; opacity:0.8;">${
                  item.path
                }</span>
            </div>
